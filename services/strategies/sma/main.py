...
async def consume_price_stream():
    ...
    async for message in pubsub.listen():
        ...
        try:
            ...
            if len(price_window[symbol]) >= MAX_WINDOW:
                df = pd.DataFrame(price_window[symbol], columns=["close"])
                short_ma = df["close"].rolling(window=5).mean().iloc[-1]
                long_ma = df["close"].rolling(window=MAX_WINDOW).mean().iloc[-1]
                ...
                print(f"📤 {symbol}: {signal} ({short_ma:.2f} vs {long_ma:.2f})")
